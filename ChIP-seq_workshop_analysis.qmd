...
::: blue
``` bash
# very slow (no need to run)
plotFingerprint -b bwa_out/*/*uniq.bam -e 200 --smartLabels -o fingerprint.png &
```
:::

The fingerprint tool can take a long time to run so if you want to run it, do so in the background (by adding '&') and continue with the tutorial. However, You should get a plot that looks like the one below where approximately 30% of the ChIP reads are concentrated in to a small number of bins.
...
``` bash
cp /home/genomes/s.cerevisiae/sacCer3/annotation/UCSC_sgdGene.bed .
```
...
``` bash
bedtools getfasta -fi /home/genomes/s.cerevisiae/sacCer3/sacCer3.fa -bed Reb1_peaks.narrowPeak -fo Reb1_peaks.fasta &
```
...
cp /home/library/training/ChIP-seq_workshop/data/*fq.gz fastq/.


##QC

##FastQ Screen
fastq_screen  --conf /home/genomes/tool_configs/fastq_screen/fastq_screen.conf fastq/$1.fq.gz --outdir fastq
##FastQC
...
mkdir bwa_out
mkdir bwa_out/$1
bwa mem -t 5 -a -R '@RG\tID:{}\tPL:ILLUMINA' -M /home/genomes/s.cerevisiae/sacCer3/bwa_indexes/sacCer3 fastq/$1.trim.fq.gz > bwa_out/$1/$1.sam
##Sort and index
...
cp /home/library/training/ChIP-seq_workshop/pipeline.sh . # Copy the pipeline into the new directory
...
::: challenge
<h2><i class="fas fa-pencil-alt"></i> Challenge:</h2>
As an additional exercise, edit the pipeline.sh script or write a new script to remove the chrXII rDNA region from our uniq.bam files.
:::

We can use bedtools to do this, you will need a BED file with the coordinates we want to remove;

``` bash
bedtools intersect -v -abam bwa_out/Reb1_R1/Reb1_R1.uniq.bam -b /datastore/home/genomes/s.cerevisiae/sacCer3/annotation/rDNA_full.bed > bwa_out/Reb1_R1/Reb1_R1.rDNA.uniq.bam
``` 

Make sure to name to our output something informative and comment your code.

The next step will be to create new bigWig files (is there anything we need to do to the BAMs before this?)

Finally, take a look at these files in IGV.
...
<!--Now that we have completed the workshop you should be able to do the following:  add summary?-->
